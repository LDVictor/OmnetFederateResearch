...
#ifndef __INET_CoAPPacket_H_
#define __INET_CoAPPacket_H_
...
/**
 * This class represents network CoAPPackets, datagrams, frames and other kinds of
 ...
 *
 * Internally, CoAPPacket stores the data in different kind of chunks. See the Chunk
 * class and its subclasses for details. All chunks are immutable in a CoAPPacket.
 * Chunks are automatically merged if possible as they are inserted into a CoAPPacket,
 * and they are also shared among CoAPPackets when duplicating.
 *
 * CoAPPackets are conceptually divided into three parts during processing: front
 * popped part, data part, and back popped part. These parts are separated by
 * iterators called front and back pointer, which are stored within the CoAPPacket.
 *
 *     +----------------------------------------------------------------+
 *     |                                                                |
 *     |             CoAPPacket content is divided into 3 parts             |
 *     |                                                                |
 *     +---------------------+---------------------+--------------------+
 *     |                     |                     |                    |
 ...
 *
 * During CoAPPacket processing, as the CoAPPacket is passed through the protocol layers
 ...
 * part, but it doesn't affect the data stored in the CoAPPacket. Popping headers and
 ...
 *  - query the length of the CoAPPacket and the data part
 *  - insert to the beginning or end of the CoAPPacket
 *  - remove from the beginning or end of the CoAPPacket
 *  - peek or pop from the front or the back of the data part
 *  - serialize to and deserialize from a sequence of bits or bytes
 *  - copy the CoAPPacket, it's a cheap operation
 *  - convert to a human readable string
 */
class INET_API CoAPPacket : public cPacket
{
  friend class CoAPPacketDescriptor;
  ...
  public:
    explicit CoAPPacket(const char *name = nullptr, short kind = 0);
    CoAPPacket(const char *name, const Ptr<const Chunk>& content);
    CoAPPacket(const CoAPPacket& other);

    virtual CoAPPacket *dup() const override { return new CoAPPacket(*this); }
    virtual void forEachChild(cVisitor *v) override;

    /** @name Length querying related functions */
    //@{
    /**
     * Returns the total CoAPPacket length ignoring front and back offsets.
    ...
    //@{
    virtual void encapsulate(cPacket *CoAPPacket) override { throw cRuntimeError("Invalid operation"); }
    ...
    //@{
    /**
     * Returns the front offset measured from the beginning of the CoAPPacket.
     * The returned value is in the range [0, getTotalLength()].
     */
    b getFrontOffset() const { return frontIterator.getPosition(); }

    /**
     * Changes the front offset measured from the beginning of the CoAPPacket.
    ...
    //@{
    /**
     * Returns the back offset measured from the beginning of the CoAPPacket.
     * The returned value is in the range [0, getTotalLength()].
     */
    b getBackOffset() const { return getTotalLength() - backIterator.getPosition(); }

    /**
     * Changes the back offset measured from the beginning of the CoAPPacket.
     * The value must be in the range [0, getTotalLength()].
     */
    ...
    //@{
    /**
     * Returns the current length of the data part of the CoAPPacket. This is the
    ...
    //@{
    /**
     * Returns the designated part of the CoAPPacket as an immutable chunk in its
    ...
    /**
     * Returns true if the designated part of the CoAPPacket is available in the
     * requested representation. If the length is unspecified, then the length
    ...
    /**
     * Returns the designated part of the CoAPPacket as an immutable chunk in the
     * requested representation. If the length is unspecified, then the length
     * of the result is chosen according to the current representation. The
     * flags parameter is a combination of Chunk::PeekFlag enumeration members.
     */
    ...
    /**
     * Returns the whole CoAPPacket (including front and back popped parts) in the
     * current representation. The length of the returned chunk is the same as
    ...
    /**
     * Returns the whole CoAPPacket (including front and back popped parts) as a
     * sequence of bits. The length of the returned chunk is the same as the
    ...
    //@{
    /**
     * Inserts the provided part at the beginning of the CoAPPacket. The inserted
    ...
    /**
     * Inserts the provided part at the end of the CoAPPacket. The inserted part is
    ...
    //@{
    /**
     * Erases the requested amount of data from the beginning of the CoAPPacket. The
     * length of front popped part must be zero before calling this function.
     */
    void eraseAtFront(b length);

    /**
     * Erases the requested amount of data from the end of the CoAPPacket. The length
     * of back popped part must be zero before calling this function.
     */
    void eraseAtBack(b length);

    /**
     * Erases all data from CoAPPacket and resets both front and back offsets to zero.
     */
    void eraseAll();

    /**
     * Erases the front popped part and sets the front offset to zero. The back
     * popped part and the data part of the CoAPPacket isn't affected.
     */
    void trimFront();
     ...
     * offsets to zero. The data part of the CoAPPacket isn't affected.
     */
    void trim();
    //@}
    ...
    /**
     * Removes all data from the CoAPPacket and returns it as a mutable chunk in the
    ...
    /**
     * Returns the number of CoAPPacket tags.
     */
    ...
    /**
     * Returns the CoAPPacket tag at the given index.
     */
    ...
    /**
     * Clears the set of CoAPPacket tags.
     */
    void clearTags() {
        tags.clearTags();
    }
    ...
    /**
     * Copies the set of CoAPPacket tags from the other CoAPPacket.
     */
    void copyTags(const CoAPPacket& source) {
        tags.copyTags(source.tags);
    }

    /**
     * Returns the CoAPPacket tag for the provided type or returns nullptr if no such CoAPPacket tag is found.
     */
    ...
    /**
     * Returns the CoAPPacket tag for the provided type or throws an exception if no such CoAPPacket tag is found.
     */
    ...
    /**
     * Returns a newly added CoAPPacket tag for the provided type, or throws an exception if such a CoAPPacket tag is already present.
     */
    ...
    /**
     * Returns a newly added CoAPPacket tag for the provided type if absent, or returns the CoAPPacket tag that is already present.
     */
    ...
    /**
     * Removes the CoAPPacket tag for the provided type, or throws an exception if no such CoAPPacket tag is found.
     */
    ...
    /**
     * Removes the CoAPPacket tag for the provided type if present, or returns nullptr if no such CoAPPacket tag is found.
     */
...
inline std::ostream& operator<<(std::ostream& os, const CoAPPacket *CoAPPacket) { return os << CoAPPacket->str(); }

inline std::ostream& operator<<(std::ostream& os, const CoAPPacket& CoAPPacket) { return os << CoAPPacket.str(); }

} // namespace

#endif // #ifndef __INET_CoAPPacket_H_
